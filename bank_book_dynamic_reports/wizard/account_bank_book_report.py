...
class AccountBankBookReport(models.TransientModel):
    """This class extends the 'account.bank.book.report' model to provide
    additional functionality for generating Bank Book Reports"""
    _inherit = 'account.bank.book.report'

    def view_report(self):
        """Generate and display the Bank Book Report.

        This method is triggered when the user clicks on the 'View Report'
        button in the Bank Book Report wizard. It fetches the necessary data,
        performs calculations, and then returns an action to display the
        generated report."""
        self.ensure_one()
        ...
        accounts_res = self.with_context(
            data['form'].get(
                'used_context', {}))._get_dynamic_move_entry(
    ...
    def _get_dynamic_move_entry(self, accounts, init_balance, sortby,
                                display_account):
        """Generate dynamic move entries for the given accounts based on the
        ...
        if init_balance:
            ...
            sql = (
                    ...
                    WHERE
                        l.account_id IN %s""" + filters + ' GROUP BY l.account_id, l.move_id'
            )
        ...
        sql = (
                ...
                WHERE
                    l.account_id IN %s ''' + filters + '''
            GROUP BY
                l.id, l.account_id, l.date, j.code, l.currency_id,
                l.amount_currency, l.ref, l.name, m.name, m.id, c.symbol, p.name
            ORDER BY
                ''' + sql_sort
        )
        ...
        for account in accounts:
            currency = account.currency_id and account.currency_id or account.company_currency_id
    ...
    def _get_currency(self):
        """Get the currency information for the current user or the default
         journal."""
