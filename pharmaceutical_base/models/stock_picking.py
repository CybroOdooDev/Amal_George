...
class StockPicking(models.Model):
    ...
    def _action_done(self):
        ...
        # The QA rejection transfer (into the Rejected area) is exempt: the lot
        # is already marked rejected, and segregating it is the whole point of
        # the move, so the blocked-status check must not stand in its way.
        if not self.env.context.get('pharma_qa_disposition'):
            for picking in self:
                if picking.picking_type_id.code != 'incoming':
                    self._check_pharma_lot_status(picking)

        # GMP: putaway only *suggests* a destination — an operator can overwrite
        # location_dest_id on a move line by hand. Checked for EVERY picking,
        # including QA dispositions: releasing a cold-chain lot into ambient
        # stock is exactly the mistake this guard exists to catch.
        for picking in self:
    ...
    def _check_pharma_storage_class(picking):
        ...
        A location provides its own storage category and every category on its
        ancestors, so a rack inside a tagged area satisfies that area's class.
        Products carrying no storage class are skipped, and so are non-internal
        destinations (customers, production, transit) which hold no storage
        conditions of their own.

        This is deliberately not an ``@api.constrains`` on ``stock.move.line``:
        that fires on draft and reserved lines and would trip on the transit
        locations of multi-step routes.
        """
        for line in picking.move_line_ids:
