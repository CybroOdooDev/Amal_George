...
class PharmaQcTestOrder(models.Model):
    ...
    def _pharma_dispose_rejected(self):
        ...
        Only a QC *failure* relocates material. Passing QC leaves the lot where
        putaway placed it, so there is no counterpart method for release.
        Putaway is resolved beneath the Rejected location, so the lot lands in
        the sub-area matching its Storage Class rather than on the area root.
        """
        self.ensure_one()
        ...
        if dest:
            lot._pharma_create_disposition_transfer(dest)
