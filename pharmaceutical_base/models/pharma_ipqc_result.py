...
class PharmaIPQCResult(models.Model):
    ...
    def _check_step_signoff_state(self):
        ...
        for rec in self:
            ...
            if step.supervisor_signed_on:
                raise ValidationError(_(
                    'Cannot create an IPQC check for step "%s" — the supervisor '
                    'has already signed off this step.'
                ) % step.name)
    ...
    def _onchange_parameter_id(self):
        _logger.info("ONCHANGE PARAMETER ID CALLED: %s", self.parameter_id)
        if self.parameter_id:
            self.expected_min = self.parameter_id.min_value
            self.expected_max = self.parameter_id.max_value
            _logger.info("ONCHANGE VALUE SET: min=%s, max=%s", self.expected_min, self.expected_max)
        else:
