...
class StockLot(models.Model):
    ...
    def _pharma_create_disposition_transfer(self, dest_location):
        ...
        Used only by a QC failure, to segregate the lot into the Rejected area.
        """
        self.ensure_one()
        ...
        # On-hand quants of this lot in internal locations, excluding only the
        # EXACT destination location so we never move stock already there.
        # NOTE: we intentionally match the destination exactly (not `child_of`).
        # The lot normally sits in a storage-class sub-area that may share an
        # ancestor with the Rejected area, and putaway drops the line into a
        # sub-area *below* the destination. A `child_of` exclusion would treat
        # such a quant as "already at destination" and skip the move entirely,
        # leaving rejected material mixed in with good stock. Matching the exact
        # destination relocates it correctly.
        ...
        if not picking_type:
            # Warehouses sometimes archive the default internal operation
            # types (Internal Transfers / Quality Control / Storage). Without
            # an internal picking type the disposition would silently no-op and
            # the rejected lot would stay mixed in with good stock, so fall back
            # to an archived internal type rather than skip the move.
        ...
        for move in picking.move_ids:
            ...
            # The generated move lines are discarded above, so native putaway
            # never runs here — resolve it explicitly or a rejected cold-chain
            # lot lands on the flat Rejected location, i.e. straight out of
            # refrigeration, and it must stay refrigerated until it is destroyed
            # or returned. ``quantity`` is passed so storage-category capacity
            # and allow_new_product are evaluated; without it a full cold room
            # would still accept the lot.
