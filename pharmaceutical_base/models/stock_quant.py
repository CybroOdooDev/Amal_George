...
class StockQuant(models.Model):
    ...
    def _compute_has_incoming_qc(self):
        """Executes the _compute_has_incoming_qc operation."""
        for quant in self:
            product_tmpl = quant.product_id.product_tmpl_id
            if not product_tmpl:
                quant.has_incoming_qc = False
                continue
            existing = self.env['pharma.qc.test.order'].search_count([
    ...
    def _compute_qc_test_order_count(self):
        ...
        for quant in self:
            product_tmpl = quant.product_id.product_tmpl_id
            quant.qc_test_order_count = self.env['pharma.qc.test.order'].search_count([
                ('product_id', '=', product_tmpl.id),
            ]) if product_tmpl else 0

    def action_view_qc_test_orders(self):
        """Open the QC Test Orders linked to this quant's product."""
        self.ensure_one()
        product_tmpl = self.product_id.product_tmpl_id
        action = self.env['ir.actions.act_window']._for_xml_id(
            'pharmaceutical_base.pharma_qc_test_order')
        action['domain'] = [('product_id', '=', product_tmpl.id)]
        action['context'] = {
            'default_product_id': product_tmpl.id,
            'default_lot_id': self.lot_id.id,
            'default_stage': 'incoming',
        }
        orders = self.env['pharma.qc.test.order'].search(
            [('product_id', '=', product_tmpl.id)])
        if len(orders) == 1:
    ...
    def action_quant_create_qc_test(self):
        ...
        for quant in self:
            ...
            if not lot:
                continue
            product_tmpl = quant.product_id.product_tmpl_id
            existing_order = self.env['pharma.qc.test.order'].search_count([
                ('product_id', '=', product_tmpl.id),
            ])
            if existing_order:
                continue
